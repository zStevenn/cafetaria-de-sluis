...
export function MenuItem(props) {
  ...
  return (
    <div className="grid grid-rows-2 grid-cols-3 grid-flow-col py-3 px-2 bg-primary my-6 rounded-lg shadow shadow-primary">
      {/* Item name */}
      <p className="col-span-2 capitalize text-white">{props.name}</p>
	{/* Description */}
	<p className="col-span-2 text-slate-300"></p>
      {/* Price */}
      <p className="place-self-center row-span-2 tracking-wide text-white">&euro; {formattedPrice}</p>
    </div>
  );
}
